module Ted.FrontEnd.Glfw ( startGlfwFrontEnd ) where
...
keyToEvent key action
  | action `elem` [G.KeyState'Pressed, G.KeyState'Repeating] && key == G.Key'Left  =  Just $ Ev.MotionBegins DirLeft
  | action `elem` [G.KeyState'Pressed, G.KeyState'Repeating] && key == G.Key'Right =  Just $ Ev.MotionBegins DirRight
  | action `elem` [G.KeyState'Pressed, G.KeyState'Repeating] && key == G.Key'Up    =  Just $ Ev.MotionBegins DirUp
  | action `elem` [G.KeyState'Pressed, G.KeyState'Repeating] && key == G.Key'Down  =  Just $ Ev.MotionBegins DirDown
  | action == G.KeyState'Released && key == G.Key'Left  = Just $ Ev.MotionEnds DirLeft
  | action == G.KeyState'Released && key == G.Key'Right = Just $ Ev.MotionEnds DirRight
  | action == G.KeyState'Released && key == G.Key'Up    = Just $ Ev.MotionEnds DirUp
  | action == G.KeyState'Released && key == G.Key'Down  = Just $ Ev.MotionEnds DirDown
  | otherwise = traceShow  ("Not handled yet: " ++ show key ++ " " ++ show action) Nothing

keyCallback :: TQueue Ev.Event -> G.KeyCallback
keyCallback queue window key scancode action mods = do
  when (key == G.Key'Escape && action == G.KeyState'Pressed) (G.setWindowShouldClose window True)
  let me = keyToEvent key action
  case me of Nothing -> return ()
...
mainLoop :: TQueue Ev.Event -> FTGL.Font -> Float -> G.Window -> EditorState -> IO (EditorState)
...
processEvents queue state = do
  ...
  case me of Nothing -> return state
             Just e -> let state' = processEvent e state
...
simpleDrawLines nlines xrat font ls (View line col ox oy) =
  let s = 1 / fi fontPoints :: GLfloat
      drawLine l = do
        preservingMatrix $ do
          scale (s * xrat) (-s) 1
          FTGL.renderFont font (drop (fi $ col - 1) l) FTGL.Front
        translate $ v3 0 1 0
  in do translate $ v3 (-ox) (-oy) 0
        mapM_ drawLine $ (take nlines . drop (fi (line - 1))) ls
        return ()
